...
public class GameManager : MonoBehaviour
{
    ...
    public TMP_Text scoreText; //posible eliminar
    ...
    [Header("Estrellas")] //atributos de las estrellas
...
/*Posible game selector o un juego en especifico*/
    [Header("Instruccion")]
...
/*Parece ser de la base de datos*/
    ...
    private void Start()//solo llama a bootstrap game
    {
        StartCoroutine(BootstrapGame());
    }

    private IEnumerator BootstrapGame()// posible configuracion inicial, instanciar a la base de datos
    {
        if (WinPanel != null)
        {
    ...
    private IEnumerator LoadActivity(int idGameActivity) // posible game selector o db
    {
    ...
    private IEnumerator SetupScene(DragDropConfig config) //Posible gamemanagerdrop
    {
    ...
    public void RespuestaCorrecta()//deberia ir solo en game and drop
    {
        correctItems++;
        RecalcularPuntaje();
        VerificarVictoria();
    ...
    public void RespuestaIncorrecta()//deberia ir solo en game and drop
    {
        wrongAttempts++;
        RecalcularPuntaje();
    }

    private void ActualizarUI()//Va con la variable que se podria eliminar
    {
        if (scoreText != null)
        {
    ...
    private void ConfigurarPuntaje()//al parecer funciona en alguna forma para validar el puntaje
    {
    ...
    private void RecalcularPuntaje()//podria ir en el game manager 
    {
        ...
        ActualizarUI();//tal vez se borre
    }

    private void VerificarVictoria()//validar victoria llama a la courutine de abajo
    {
    ...
    public void MostrarVictoria(int scoreValue, int maxScoreValue)//hace otra validacion para llamar otro metodo
    {
        StartCoroutine(MostrarVictoriaLegacy(scoreValue, maxScoreValue));
    }

    private IEnumerator MostrarVictoria()//actualiza las estrellas y guarda infromacion a la base de datos
    {
    ...
    private IEnumerator MostrarVictoriaLegacy(int scoreValue, int maxScoreValue)//supongo que configura algo de las estrellas
    {
        yield return new WaitForSeconds(1f);
        yield return StartCoroutine(AnimarPanelVictoria());
    ...
    private void ActualizarEstrellas()//Organiza las estrellas ganadas
    {
        int estrellasGanadas = CalcularEstrellasGanadas();
        PrepararPanelDeEstrellas(estrellasGanadas);
    }

    private int CalcularEstrellasGanadas()//el nombre lo dice
    {
    ...
    private void PrepararPanelDeEstrellas(int estrellasGanadas)//otro que prepara las estrellas
    {
    ...
    private void LimpiarEstrellasGeneradas()//supongo que es una forma de borrar Cache
    {
    ...
    private Vector2 ObtenerPosicionEstrella(int index, int totalStars)//Una forma de organizar las estrellas
    {
    ...
    private string ObtenerTiempoCompletado()//tiempo del jugador
    {
    ...
    private IEnumerator AnimarPanelVictoria()//el nombre lo dice
    {
        if (WinPanel == null)
        {
            yield break;
    ...
    private void ResetGameState()//el nombre lo dice
    {
        score = 0;
        totalItems = 0;
        totalZones = 0;
    ...
    private void OnDestroy()//su ongo que se ejecuta cuando se ba a destruir
    {
        if (instance == this)
        {
            instance = null;
    ...
    private void EnsureDatabaseManagerExists()//algo de la base de datos
    {
    ...
    private void ResolveSceneReferences()//creo que algo que relaciona los dos game manager
    {
        if (scoreText == null)
        {
    ...
    private TMP_Text FindTextByNameHint(params string[] hints)//no estoy seguro
    {
    ...
    private int NormalizeGameActivityId(int gameActivityId)//creo que tiene que ver con el game selector
    {
    ...
    private void ResolveLoggedInUser()//algo de la base de datos
    {
